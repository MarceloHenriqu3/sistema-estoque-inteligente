...
app.MapPost("/api/categories", async (CategoryDto dto, InventoryContext db) =>
{
...
app.MapPut("/api/categories/{id}", async (int id, CategoryDto dto, InventoryContext db) =>
{
    var name = dto.Name?.Trim();
    if (string.IsNullOrWhiteSpace(name))
        return Results.BadRequest("Nome da categoria é obrigatório.");
...
app.MapDelete("/api/categories/{id}", async (int id, InventoryContext db) =>
{
    var category = await db.Categories.FindAsync(id);
    if (category == null) return Results.NotFound();
...
app.MapPost("/api/users/register", async (UserRegisterDto dto, InventoryContext db, HttpRequest request) =>
{
    var userRole = request.Headers["X-User-Role"].ToString();
    if (string.IsNullOrWhiteSpace(userRole) || !userRole.Equals("Administrador", StringComparison.OrdinalIgnoreCase))
    ...
    return Results.Created($"/api/users/{user.Id}", new { user.Id, user.Username, user.Name, user.Role });
});

app.MapPost("/api/users/login", async (LoginDto dto, InventoryContext db) =>
{
    if (string.IsNullOrWhiteSpace(dto.Username) || string.IsNullOrWhiteSpace(dto.Password))
        return Results.BadRequest("Usuário e senha são obrigatórios.");
...
app.MapGet("/api/users", async (InventoryContext db, HttpRequest request) =>
{
    if (!IsAdminRequest(request)) return Results.Forbid();

    var users = await db.Users
        .OrderBy(u => u.Username)
        .Select(u => new { u.Id, u.Username, u.Name, u.Role, u.IsActive })
        .ToListAsync();
    return Results.Json(users);
});

app.MapPut("/api/users/{id}", async (int id, UserUpdateDto dto, InventoryContext db, HttpRequest request) =>
{
    if (!IsAdminRequest(request)) return Results.Forbid();
...
app.MapPut("/api/users/{id}/status", async (int id, UserStatusDto dto, InventoryContext db, HttpRequest request) =>
{
    if (!IsAdminRequest(request)) return Results.Forbid();
...
app.MapPut("/api/users/{id}/password", async (int id, PasswordResetDto dto, InventoryContext db, HttpRequest request) =>
{
    if (!IsAdminRequest(request)) return Results.Forbid();
...
app.MapPost("/api/products", async (Product p, InventoryContext db) =>
{
    p.Name = p.Name.Trim();
...
app.MapPut("/api/products/{id}", async (int id, Product updatedProduct, InventoryContext db) =>
{
...
app.MapPut("/api/products/{id}/active", async (int id, ProductStatusDto dto, InventoryContext db) =>
{
    var product = await db.Products.FindAsync(id);
    if (product == null) return Results.NotFound();
...
app.MapPost("/api/movements", async (MovementDto dto, InventoryContext db) =>
{
    ...
    var operatorName = string.IsNullOrWhiteSpace(dto.Operator) ? "Sistema" : dto.Operator.Trim();
    var m = new Movement
    {
...
app.MapPost("/api/ai/test-history", async (AiTestHistoryDto dto, InventoryContext db, HttpRequest request) =>
{
    if (!IsAdminRequest(request)) return Results.Forbid();
...
bool IsAdminRequest(HttpRequest request)
{
    var userRole = request.Headers["X-User-Role"].ToString();
    return userRole.Equals("Administrador", StringComparison.OrdinalIgnoreCase);
}
